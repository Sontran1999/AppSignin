        ...
        edt_pass.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(p0: Editable?) {
                if (edt_pass?.text.toString().length > 17) {
                    edt_pass.setText(
                        edt_pass?.text.toString().substring(0, edt_pass?.text.toString().length - 1)
                    )
        ...
        when (p0?.id) {
            R.id.btn_signin -> {
                ...
                if (email.isEmpty()) {
                    ...
                    var userEmail: User? =
                        userDB?.getDao()?.findUserByEmail(edt_email.text.toString())
                    println(userEmail.toString() + ", " + edt_email.text.toString())

                    if (userEmail != null) {
                        var userPass: User? =
                            userDB?.getDao()?.findUserByPass(edt_pass.text.toString())
                        if (userPass != null) {
